...
const TreeItem = React.memo(({ id, nodes, dispatch, focusId, focusCursorPos, matched, isFiltering, searchQuery, onDeleteRequest, draggingId, dragOverId, onDragStart, onDragOver, onDragEnd, onDrop }: TreeItemProps) => {
  ...
  // 設計方針: inputをテキストdivの中にabsolute配置し、overflow:hiddenでタッチ領域をdiv内にクリップ
  // iOSはoverflow:hiddenを尊重してヒットテストをクリップするため隣ボタンへの干渉がなくなる
  const mobileDateArea = (
    <div className="flex items-center gap-0.5 text-xs">
      {/* 開始日: overflow-hiddenでinputのiOSタッチ領域をdiv内に封じ込める */}
      <div className="flex items-center">
        <div className={`relative overflow-hidden min-w-[28px] px-0.5 text-center rounded cursor-pointer select-none ${
      ...
      <header className="flex-shrink-0 bg-white/90 backdrop-blur-sm border-b border-gray-200 shadow-sm">
        <div className="w-full max-w-5xl mx-auto px-4 sm:px-8 py-2 flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            <img src="/api/icon?size=180" alt="Outliner" className="w-[29px] h-[29px] rounded-lg border border-gray-200 flex-shrink-0 ml-0.5" />
